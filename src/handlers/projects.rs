...
use crate::models::{project, status, ticket_type, user};
...
/// Can create projects: admin or users with is_manager flag
fn can_create_project(user: &AuthenticatedUser) -> bool {
    user.is_admin() || user.is_manager()
}

/// Can manage a specific project (edit, members, config): admin or manager role in that project
fn can_manage_project(conn: &rusqlite::Connection, user: &AuthenticatedUser, project_id: i64) -> bool {
    user.is_admin() || project::get_member_role(conn, project_id, user.id).as_deref() == Some("manager")
}

/// Can delete a project: admin or owner (is_manager flag + created_by)
fn can_delete_project(conn: &rusqlite::Connection, user: &AuthenticatedUser, project_id: i64) -> bool {
    if user.is_admin() {
        return true;
    }
    if !user.is_manager() {
